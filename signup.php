	...
	if (isset($_POST['submit'])) {	
		...
		&& ($u_pass1==$u_pass2)){
			...
			if(mysqli_num_rows($data)==0){
				//username is unique, append to user table
				$query="INSERT INTO users (u_name, u_pass) VALUES ('$u_name', SHA('$u_pass1'))";
				mysqli_query($dbc, $query) or die('Error inserting into user table');
				...
				//create user's individual tweet bank
				$query="CREATE TABLE tweets_of_user_$u_id (t_id int NOT NULL," . 
						" t_update_time timestamp NOT NULL DEFAULT NOW(), UNIQUE (t_id))";
				mysqli_query($dbc, $query) or die('Error creating user\'s tweet bank');
